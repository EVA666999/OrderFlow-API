...
from api_django.models import Category, Product, ProductReview, Order, Discount
...
def get_token_for_user(user_employee, user_customer, user_supplier):
    ...
    response = client.post(
        '/auth/jwt/create/', 
        {
            "email": user_supplier[0]["email"],
            "password": user_supplier[0]["password"]
        },
        format='json'
    )
    assert response.status_code == status.HTTP_200_OK
...
def test_patch_category_as_employee(get_token_for_user):
    ...
    patch_response = client.patch(f'/api/category/{category_id}/', update_data, format='json')
...
def test_patch_product_as_supplier(get_token_for_user, category):
    ...
    patch_response = client.patch(f'/api/products/{product_id}/', update_data, format='json')
...
def test_patch_order_as_customer(get_token_for_user, product1, product2, discount):
    ...
    patch_response = client.patch(f'/api/orders/{order_id}/', update_data, format='json')
...
def test_patch_review_as_customer(get_token_for_user, product1):
    ...
    patch_response = client.patch(f'/api/reviews/{review_id}/', update_data, format='json')
...
def test_patch_discount_as_employee(get_token_for_user):
    ...
    patch_response = client.patch(f'/api/discounts/{discount_id}/', update_data, format='json')
